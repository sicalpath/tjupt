...
//AJAX related actions 2012/09/13
if ($action == 'ajaxtick')
{
	header("Cache-Control: no-cache, must-revalidate");
	header("Pragma: no-cache");
	$start = 0 + $_GET['start'];
	$res = sql_query("SELECT m.*, u.username AS sender_name FROM messages m LEFT JOIN users u ON u.id = m.sender WHERE m.receiver = " . sqlesc($CURUSER['id']) . " AND m.unread = 'yes' AND m.location = '1' AND m.id > '{$start}' ORDER BY m.`added` ASC") or sqlerr(__FILE__,__LINE__);
	$result = array();
	while ($row = mysql_fetch_array($res))
	{
		$result[] = array('id' => $row['id'], 'sender_name' => $row['sender_name'], 'sender_uid' => $row['sender'], 'subject' => $row['subject'], 'msg' => format_comment($row['msg']), 'added' => $row['added']);
	}
...
if ($action == 'ajaxread')
{
	header("Cache-Control: no-cache, must-revalidate");
	header("Pragma: no-cache");
	sql_query("UPDATE messages SET unread = 'no' WHERE receiver = " . sqlesc($CURUSER['id']) . " AND id = " . sqlesc($_GET['id']) . " LIMIT 1") or sqlerr(__FILE__,__LINE__);
	mysql_affected_rows() > 0 && $Cache->delete_value('user_'.$CURUSER['id'].'_unread_message_count');
...
// Check if messages were moved
	...
	header("Location: messages.php?action=viewmailbox&box=" . $pm_box.(is_numeric($_POST["page"])&&$_POST["page"]!=0?"&page=".$_POST["page"]:"").(in_array($_POST["unread"],array("yes","no"))?"&unread=".$_POST["unread"]:""));
	exit();
}
elseif ($_POST['move'])
{
...
function messagemenu ($selected = 1) {
	...
	print ("<li" . ($selected == 1 ? " class=selected" : "") . "><a href=\"messages.php\" >".$lang_messages['text_inbox']."</a></li>");
	print ("<li" . ($selected == -1 ? " class=selected" : "") . "><a href=\"messages.php?action=viewmailbox&box=-1\">".$lang_messages['text_sentbox']."</a></li>");
	$res = sql_query('SELECT * FROM pmboxes WHERE userid=' . sqlesc($CURUSER['id'])) or sqlerr(__FILE__,__LINE__);
	if (mysql_num_rows($res))
		while ($row = mysql_fetch_assoc($res))
		{
		print ("<li" . ($selected == $row['boxnumber'] ? " class=selected" : "") . "><a href=\"messages.php?action=viewmailbox&box=".$row['boxnumber']."\">".$row['name']."</a></li>");
		}
	print ("</ul></div>");
	end_main_frame();
}
