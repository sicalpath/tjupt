...
if ($action == "add") {
	...
	begin_compose ( $title, ($sub == "quote" ? "quote" : "reply"), ($sub == "quote" ? htmlspecialchars ( "[quote=" . htmlspecialchars ( $arr2 ["username"] ) . "]" . unesc ( $arr2 ["text"] ) . "[/quote] @" . htmlspecialchars ( $arr2 ["username"] ) . " " ) : ""), false );
	end_compose ();
	print ("</form>") ;
	end_main_frame ();
	stdfoot ();
