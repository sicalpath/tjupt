...
function schoolstable($res, $frame_caption, $what)
{
	global $CURUSER;
	global $lang_topten;
	begin_frame($frame_caption, true);
...
<tr>
<td class="colhead"><?php echo $lang_topten['col_rank'] ?></td>
<td class="colhead" align="left"><?php echo $lang_topten['col_school'] ?></td>
<td class="colhead" align="right"><?php echo $what?></td>
</tr>
<?php
$num = 0;
while ($a = mysql_fetch_assoc($res))
{
	...
	elseif ($what == $lang_topten['col_ratio'])
	$value = number_format($a["r"],2);
	print("<tr><td class=\"rowfollow\" align=\"center\">$num</td><td class=\"rowfollow\" align=\"left\"><table border=\"0\" class=\"main\" cellspacing=\"0\" cellpadding=\"0\"><tr><td class=\"embedded\" style='padding-left: 5px'><b>$a[name]</b></td>".
	"</tr></table></td><td class=\"rowfollow\" align=\"right\">$value</td></tr>\n");
}
end_table();
end_frame();
...
function invitestable($res, $frame_caption)
{
	global $lang_topten;
	begin_frame($frame_caption, true);
	begin_table();

	print("<tr><td class=\"colhead\">".$lang_topten['col_rank']."</td><td class=\"colhead\">".$lang_topten['col_username']."</td><td class=\"colhead\">".$lang_topten['col_invites']."</td></tr>");

	$n = 1;
	while ($arr = mysql_fetch_assoc($res))
	{
		//die();
		print("<tr><td class=\"rowfollow\" align=\"center\">$n</td><td class=\"rowfollow\" align=\"left\">" . get_username($arr["id"]) . "</td><td class=\"rowfollow\" align=\"right\">" . $arr["invites"] . "</td></tr>\n");
		$n++;
	}
...
("<a href=\"imdb_top_250.php\">IMDb Top 250</a>") .	" | " .
...
($type == 8 && !$limit ? "<b>".$lang_topten['text_schools']."</b>" : "<a href=\"topten.php?type=8\">".$lang_topten['text_schools']."</a>")  . " | " .
...
elseif ($type == 8)
{
	if ($limit == 10 || $subtype == "us")
	{
		$r = sql_query("SELECT name, COUNT(users.school) as num FROM schools LEFT JOIN users ON users.school = schools.id GROUP BY name ORDER BY num DESC LIMIT $limit") or sqlerr();
		schoolstable($r, $lang_topten['text_top']."$limit ".$lang_topten['text_schools_users']. ($limit == 10 ? " <font class=\"small\"> - [<a class=\"altlink\" href=\"topten.php?type=$type&amp;lim=25&amp;subtype=us\">Top 25</a>]</font>" : ""),$lang_topten['col_users']);
	}

	if ($limit == 10 || $subtype == "ul")
	{
		$r = sql_query("SELECT c.name, sum(u.uploaded) AS ul FROM users AS u LEFT JOIN schools AS c ON u.school = c.id WHERE u.enabled = 'yes' GROUP BY c.name ORDER BY ul DESC LIMIT $limit") or sqlerr();
		schoolstable($r, $lang_topten['text_top']."$limit ".$lang_topten['text_schools_uploaded']. ($limit == 10 ? " <font class=\"small\"> - [<a class=\"altlink\" href=\"topten.php?type=$type&amp;lim=25&amp;subtype=ul\">Top 25</a>]</font>" : ""),$lang_topten['col_uploaded']);
	}

	if ($limit == 10 || $subtype == "avg")
	{
		$r = sql_query("SELECT c.name, sum(u.uploaded)/count(u.id) AS ul_avg FROM users AS u LEFT JOIN schools AS c ON u.school = c.id WHERE u.enabled = 'yes' GROUP BY c.name HAVING sum(u.uploaded) > 1099511627776 AND count(u.id) >= 10 ORDER BY ul_avg DESC LIMIT $limit") or sqlerr();
		schoolstable($r, $lang_topten['text_top']."$limit ".$lang_topten['text_schools_per_user']. ($limit == 10 ? " <font class=\"small\"> - [<a class=\"altlink\" href=\"topten.php?type=$type&amp;lim=25&amp;subtype=avg\">Top 25</a>]</font>" : ""),$lang_topten['col_average']);
	}

	if ($limit == 10 || $subtype == "r")
	{
		$r = sql_query("SELECT c.name, sum(u.uploaded)/sum(u.downloaded) AS r FROM users AS u LEFT JOIN schools AS c ON u.school = c.id WHERE u.enabled = 'yes' GROUP BY c.name HAVING sum(u.uploaded) > 1099511627776 AND sum(u.downloaded) > 1099511627776 AND count(u.id) >= 10 ORDER BY r DESC LIMIT $limit") or sqlerr();
		schoolstable($r, $lang_topten['text_top']."$limit ".$lang_topten['text_schools_ratio']. ($limit == 10 ? " <font class=\"small\"> - [<a class=\"altlink\" href=\"topten.php?type=$type&amp;lim=25&amp;subtype=r\">Top 25</a>]</font>" : ""),$lang_topten['col_ratio']);
	}
}
/*
elseif ($type == 4)
...
elseif ($type == 6)
{
	...
	if ($limit == 10 || $subtype == "invites")
	{
		$r = sql_query("SELECT * FROM users ORDER BY invites DESC LIMIT $limit") or sqlerr();
		invitestable($r, $lang_topten['text_top']."$limit ".$lang_topten['text_most_invites'] . ($limit == 10 ? " <font class=\"small\"> - [<a class=\"altlink\" href=\"topten.php?type=$type&amp;lim=100&amp;subtype=invites\">".$lang_topten['text_one_hundred']."</a>] - [<a class=\"altlink\" href=\"topten.php?type=$type&amp;lim=250&amp;subtype=invites\">".$lang_topten['text_top_250']."</a>]</font>" : ""));
	}
