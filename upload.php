...
{
	$offer = "<select name=\"offer\" id=\"browseoffer\" onChange=\"getuploadinfo('class3','browseoffer')\"><option value=\"0\">".$lang_upload['select_choose_one']."</option>";
	while($offerrow = mysql_fetch_array($offerres))
		$offer .= "<option value=\"" . $offerrow["id"] . "\">" . htmlspecialchars($offerrow["name"]) . "</option>";
	$offer .= "</select>".$lang_upload['text_please_select_offer']."<div id=class3></div>";
?>

<form id="compose" enctype="multipart/form-data" action="takeuploadoffer.php" method="post" name="upload">
<?php
...
<table border="1" cellspacing="0" cellpadding="5" width="940">
	...
	<?php					
		tr($lang_upload['row_your_offer']. (!$uploadfreely && !$allowspecial ? "<font color=red>*</font>" : ""), $offer , 1);
	?>		
</table>
</form>
<?php 
...
else
{
			...
			<form id="compose" enctype="multipart/form-data" action="takeupload.php" method="post" name="upload" onsubmit="return validate('browsecat')">
			<table border="1" cellspacing="0" cellpadding="5" width="940">
				<tr>
					<td class='colhead' colspan='2' align='center'>
						<?php echo $lang_upload['text_tracker_url'] ?>: &nbsp;&nbsp;&nbsp;&nbsp;<b><?php echo  get_protocol_prefix() . $announce_urls[0]?></b>
...
//				if(get_user_class() >= 12 ) 
				...
				if ($allowtorrents){
					...
					$get = " onChange=\"getcategory('class2','browsecat')\"";
					$s = "<select name=\"type\" id=\"browsecat\" ".($allowtwosec ? $disablespecial : "").$get.">\n<option value=\"0\">".$lang_upload['select_choose_one']."</option>\n";
