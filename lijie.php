<?php
$zerodayServerUsername="we";
$zerodayServerPassword="we";
...
'/ziliao/' =>"catid=404&ename=".$file,
'/soft/' =>"catid=408&ename=".file,
'/documentary/' =>"catid=411&ename=".file,
);
...
wget -nH -t0 -T15 -w3 --retry-connrefused --cut-dirs=2 -r -N -l inf -nv -R sfv -a \"".$logpath.$file."\" \"ftp://we:we@219.243.47.154:2121".$dir.$file."\" >>\"".$logpath.$file."\" 2 >>\"".$logpath.$file."\"
wget -nH -t0 -T15 -w3 --retry-connrefused --cut-dirs=2 -r -N -l inf -nv -R sfv -a \"".$logpath.$file."\" \"ftp://we:we@219.243.47.154:2121".$dir.$file."\" >>\"".$logpath.$file."\" 2 >>\"".$logpath.$file."\"
...
";
else if(substr($dir,0,10)=="/mp3/epop/"||substr($dir,0,10)=="/mp3/cpop/")
{
...
cd ".$downloadpath."
echo \"从0day服务器下载资源\">>\"".$logpath.$file."\"
wget -nH -t0 -T15 -w3 --retry-connrefused --cut-dirs=3 -r -N -l inf -nv -R sfv -a \"".$logpath.$file."\" ftp://we:we@219.243.47.154:2121".$dir.$file2." >>\"".$logpath.$file."\" 2 >>\"".$logpath.$file."\"
wget -nH -t0 -T15 -w3 --retry-connrefused --cut-dirs=3 -r -N -l inf -nv -R sfv -a \"".$logpath.$file."\" ftp://we:we@219.243.47.154:2121".$dir.$file2." >>\"".$logpath.$file."\" 2 >>\"".$logpath.$file."\"
...
elseif(in_array($dir,array("/documentary/","/soft/","/ziliao/","/apps/","/mp3/","/pcgame/","/private/","/psp/","/tv/","/tv.x264/","/x264/","/xvid/")))
$script ="#!/bin/bash
#从0day服务器下载资源
cd ".$downloadpath."
echo \"从0day服务器下载资源\">>\"".$logpath.$file."\"
wget -nH -t0 -T15 -w3 --retry-connrefused --cut-dirs=1 -r -N -l inf -nv -R sfv -a \"".$logpath.$file."\" ftp://we:we@219.243.47.154:2121".$dir.$file." >>\"".$logpath.$file."\" 2 >>\"".$logpath.$file."\"
wget -nH -t0 -T15 -w3 --retry-connrefused --cut-dirs=1 -r -N -l inf -nv -R sfv -a \"".$logpath.$file."\" ftp://we:we@219.243.47.154:2121".$dir.$file." >>\"".$logpath.$file."\" 2 >>\"".$logpath.$file."\"
...
";
	...
	//ftp操作。
	$conn_0day = ftp_connect("219.243.47.154",2121,90) or stdmsg("连接失败", "无法连接到219.243.47.154");
	ftp_login($conn_0day, $zerodayServerUsername, $zerodayServerPassword)or stdmsg("登录失败", "无法登录到219.243.47.154");
	if(substr($dir,0,6)=="/anime"){
			ftp_chdir($conn_0day, $dir );$temp=ftp_size($conn_0day, $file);
	}else {
		ftp_chdir($conn_0day, $dir.$file );
		$temp = ftp_pwd($conn_0day);
		}
	ftp_close($conn_0day);
	...
	stdmsg("错误", "貌似219.243.47.154上不存在你刚刚输入的资源(".$dir.$file.")");
	}
	echo "	<input type=\"button\" value=\"返回\" alt=\"返回\" onclick=\"javascript:window.location.href ='lijie.php'\" /> </input>";
	stdfoot();
}
else{
...
<tr><td align=center><form action=\"lijie.php\" method=\"post\">
<select name=dir>
...
5.可以到<a class=faqlink href=http://59.67.33.217/autoseed><b>http://59.67.33.217/autoseed</b></a>查看发布日志，如果是路径错误可以重新发布。<br/>
