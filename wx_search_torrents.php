...
if (! empty ( $qstr )) {
	...
	$wherea = implode ( " AND ", $like_expression_array );
	$where = "WHERE " . $wherea . " AND pulling_out = '0'";
	$orderby = "ORDER BY torrents.added DESC";
	$limit = "LIMIT 3";
	$query = "SELECT torrents.name AS tname, torrents.category, torrents.size, torrents.added, categories.name AS cname FROM torrents LEFT JOIN categories ON torrents.category=categories.id $where $orderby $limit";
	...
	while ( $item = mysql_fetch_array ( $res ) ) {
		$result [] = array (
				...
				'cat' => $item ['cname'],
				'time' => $item ['added'],
				'size' => mksize ( $item ['size'] ) 
		);
	}
}
if( empty($result) ) {
	$where = "WHERE pulling_out = '0'";
	$orderby = "ORDER BY torrents.added DESC";
	$limit = "LIMIT 3";
	$query = "SELECT torrents.name AS tname, torrents.category, torrents.size, torrents.added, categories.name AS cname FROM torrents LEFT JOIN categories ON torrents.category=categories.id $where $orderby $limit";
