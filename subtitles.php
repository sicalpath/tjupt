...
if ($search != '')
{
	$query = "subs.title LIKE " . sqlesc("%" . str_replace(".", " ", $search) . "%") . " OR subs.title LIKE " . sqlesc("%" . str_replace(" ", ".", $search) . "%");
	if ($search)
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && $_POST["action"] == "upload" && ($in_detail!= 'in_detail'))
{
	...
	*/
	...
	else
	{
		...
		{
			$r_a = mysql_fetch_assoc($r);			$r_torrent_name = $r_a["name"];
	...
	*/
	...
	// 更新种子的字幕状态
	sql_query("UPDATE torrents SET name = REPLACE(name, '[暂无字幕]', '[$arr[lang_name]字幕]') WHERE id = ". sqlesc($torrent_id)) or sqlerr(__FILE__, __LINE__);	
	write_log("$anon 上传了 $arr[lang_name] 字幕 $id ($title) ");	
	$msg_bt = "$anon 上传了 $arr[lang_name] 字幕 $id ($title) ,下载地址： " . get_protocol_prefix() . "$BASEURL/downloadsubs.php/".$file["name"]."";
}

if (get_user_class() >= $delownsub_class)
{	
	//删除字幕
	$delete_raw = $_GET["delete"];
	if ($delete_raw && !is_array($delete_raw))
	{
		$delete_raw = array($delete_raw);
	}
	if(is_array($delete_raw))
	{
		foreach ($delete_raw as $delete)
		{
				...
				if (mysql_num_rows($r) == 1)
				{
					...
					if (get_user_class() >= $submanage_class || $a["uppedby"] == $CURUSER["id"])
					{
						...
						if ($sure == 1)
						{
							...
							write_log(($a["uppedby"] != $CURUSER["id"] ? "管理员 ".$CURUSER['username']:(($a["anonymous"] == 'yes' && $a["uppedby"] == $CURUSER["id"]) ? "匿名发布者" :"发布者 ".$CURUSER['username']))." 删除了 "."$arr[lang_name]字幕 $delete ($a[title]) ".($reason != "" ? " (".$reason.")" : ""));
						}
						else
						{
							$delete = '';
							foreach ($delete_raw as $d)
							{
								$delete .= "delete[]=$d&";
							}
							stdmsg($lang_subtitles['std_delete_subtitle'], $lang_subtitles['std_delete_subtitle_note']."<br /><form method=post action=subtitles.php?{$delete}sure=1>".$lang_subtitles['text_reason_is']."<input type=text style=\"width: 200px\" name=reason><input type=submit value=\"".$lang_subtitles['submit_confirm']."\"></form>");
							stdfoot();
							die;
						}
					}
...
// 修改字幕名
	$rename_raw = $_GET["rename"];
	if ($rename_raw && !is_array($rename_raw))
	{
		$rename_raw = array($rename_raw);
	}
	if(is_array($rename_raw))
	{
		foreach ($rename_raw as $rename)
		{
			if (is_valid_id($rename))
			{
				$r = sql_query("SELECT id,torrent_id,ext,lang_id,title,filename,uppedby,anonymous FROM subs WHERE id=".sqlesc($rename)) or sqlerr(__FILE__, __LINE__);
				$a = mysql_fetch_assoc($r);
				if (mysql_num_rows($r) == 1)
				{
					...
					if (get_user_class() >= $submanage_class || $a["uppedby"] == $CURUSER["id"])
					{
						...
						if ($sure == 1)
						{
							...
							$rename1 = trim($_POST["rename1"]);
							$rename2 = substr($rename1, 0, strrpos($rename1, "."));
							$rename3 = str_replace(" ", "_", htmlspecialchars("$rename2"));
							$result=sql_query("UPDATE subs set title= '$rename3' , filename='$rename1' WHERE id=$rename") or sqlerr(__FILE__, __LINE__);

							/*
							if ($CURUSER['id'] != $a['uppedby']){
								$msg = $CURUSER['username'].$lang_subtitles_target[get_user_lang($a['uppedby'])]['msg_deleted_your_sub']. $a['title'].($rename1 != "" ? $lang_subtitles_target[get_user_lang($a['uppedby'])]['msg_reason_is'].$rename1 : "");
								$subject = $lang_subtitles_target[get_user_lang($a['uppedby'])]['msg_your_sub_deleted'];
								$time = date("Y-m-d H:i:s");
								sql_query("INSERT INTO messages (sender, receiver, added, msg, subject) VALUES(0, $a[uppedby], '" . $time . "', " . sqlesc($msg) . ", ".sqlesc($subject).")") or sqlerr(__FILE__, __LINE__);
							} */
						...
						{
							$rename = '';
							foreach ($rename_raw as $d)
							{
								$rename .= "rename=$d&";
							}
							stdmsg($lang_subtitles['std_rename_subtitle'], $lang_subtitles['std_rename_subtitle_note']."<br /><form method=post action=subtitles.php?{$rename}sure=1>".$lang_subtitles['text_rename_is']."<input type=text style=\"width: 500px\" name=\"rename1\" value=\"".$a["filename"]."\"><input type=submit value=\"".$lang_subtitles['submit_confirm']."\"></form>");
							stdfoot();
							die;
						}
					}
...
if(get_user_class() >= UC_PEASANT)
{
		...
		if (get_user_class() >= $delownsub_class)
		{
			print("<form id=\"delform\" action=\"?\" method=\"GET\" onsubmit=\"return confirm('确认删除？');\">");
		}
		...
		while ($arr = mysql_fetch_assoc($res))
		{
			...
			if (get_user_class() >= $delownsub_class)
			{
				$title .= "<input type=\"checkbox\" name=\"delete[]\" value=\"{$arr['id']}\" />";
			}
		
			$title .= "<a href=\"" . ($arr['id'] <= $start_subid ?  "downloadsubs_legacy.php/" . $arr['filename'] : "downloadsubs.php?torrentid=" . $arr['torrent_id'] ."&subid=" .$arr['id']) . "\"<b>" . htmlspecialchars($arr["title"]) . "</b></a>" .
			($arr['id'] <= $start_subid ?  "": "&nbsp;|&nbsp;<a href=\"details.php?id=" . $arr['torrent_id'] ."&hit=1" . "\"<b>[details]</b></a>&nbsp;|&nbsp;") .
			($mod || ($pu && $arr["uppedby"] == $CURUSER["id"]) ? " <font class=small><a href=?delete=$arr[id]>".$lang_subtitles['text_delete']."</a>&nbsp;|&nbsp;<a href=?rename=$arr[id]>".$lang_subtitles['text_rename']."</font>" : "") ."</td>\n";
		...
		if (get_user_class() >= $delownsub_class)
		{
			print("<script type=\"text/javascript\">function set_checked_torrent(val){var checkboxs=document.getElementsByName(\"delete[]\"); for (var i=0; i<checkboxs.length; i++) checkboxs[i].checked=val; }</script>");
			print("<a href=\"#\" onclick=\"set_checked_torrent(true); return false;\">全选</a> <a href=\"#\" onclick=\"set_checked_torrent(false); return false;\">全不选</a>, 选中项: <input type=\"submit\" value=\"删除\"></form>");
		}
		print($pagerbottom);
}
