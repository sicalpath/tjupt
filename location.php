...
if($edited == 1) {
	...
	if(validip_format($start_ip) && validip_format($end_ip))
	{
		if(ip2long($end_ip) >= ip2long($start_ip))
		{
			...
			$privilege = $privilege < 0 ? 0 : ( $privilege > 10 ? 10 : $privilege );
			$query = "UPDATE locations SET name = " . sqlesc($name) .",flagpic = " . sqlesc($flagpic) . ",location_main = " . sqlesc($location_main). ",location_sub= " . sqlesc($location_sub) . ",start_ip = " . sqlesc($start_ip) .  ",end_ip = " . sqlesc($end_ip) . ",theory_upspeed = " . sqlesc($theory_upspeed) .  ",practical_upspeed = " . sqlesc($practical_upspeed) .  ",theory_downspeed = " . sqlesc($theory_downspeed) .  ",practical_downspeed = " . sqlesc($practical_downspeed) .  ",privilege = " . sqlesc($privilege) . " WHERE id=".sqlesc($id);
...
if($add == 'true') {
	...
	if(validip_format($start_ip) && validip_format($end_ip))
	{
		if(ip2long($end_ip) >= ip2long($start_ip))
		{
			$privilege = 0 + $privilege;
			$privilege = $privilege < 0 ? 0 : ( $privilege > 10 ? 10 : $privilege );
			$query = "INSERT INTO locations (name, flagpic, location_main, location_sub, start_ip, end_ip, theory_upspeed, practical_upspeed, theory_downspeed, practical_downspeed, privilege) VALUES (" . sqlesc($name) ."," . sqlesc($flagpic) . "," . sqlesc($location_main). "," . sqlesc($location_sub) . "," . sqlesc($start_ip) .  "," . sqlesc($end_ip) . "," . sqlesc($theory_upspeed) .  "," . sqlesc($practical_upspeed) .  "," . sqlesc($theory_downspeed) .  "," . sqlesc($practical_downspeed) . "," . sqlesc($privilege) . ")";
			$sql = sql_query($query)  or sqlerr(__FILE__, __LINE__);
			if($sql) {
				$success = true;
			} else {
...
$query = "SELECT * FROM locations " . $wherea ." ORDER BY start_ip ASC, name ASC ".$limit;
$sql = sql_query($query);
...
while ($row = mysql_fetch_array($sql)) {
	...
	"<td class=rowfollow align=left><span style=\"font-weight: bold; color: \\#f00;\">" . $privilege . "</span></td>" . 
