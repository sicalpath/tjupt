...
function usercpmenu ($selected = "home") {
	...
	print ("<li" . ($selected == "usercss" ? " class=selected" : "") . "><a href=\"addcss.php\">".$lang_usercp['text_usercss_settings']."</a></li>");
	print ("</ul></div>");
	end_main_frame();
}
...
if ($CURUSER[id]==38630){
	stderr($lang_usercp['std_error'], $lang_usercp['permissiondenied'].goback(), 0);
	die;
}
	...
	switch ($action) {
		case "personal":
			if ($type == 'save') {
				...
				$updateset[] = "acceptatpms = " . sqlesc($acceptatpms);
  ...
  break;
		case "tracker":
			...
			function browsecheck($dbtable = "categories", $cbname = "cat"){
				...
				if (isset($incldead) && $incldead != 0)
					$notifs .= "[incldead=".$incldead."]";
				if($_POST["showneedseeding"] != 'yes')
				$notifs .= "[closeneedseeding]";
				if($_POST["showsearchbox"] != 'yes')
				$notifs .= "[closesearchbox]";
				...
				$updateset[] = "width = " . sqlesc($screenwidth);
...
/***********************�����ʼ�������**************************/
	...
	elseif (strpos($CURUSER['notifs'], "[spstate=6]") !== false)
		$special_state = 6;
			...
			tr_small('屏幕宽度', "<input type=radio name=screenwidth ".($CURUSER['width'] == 'wide' ? " checked" : "")." value=wide>宽屏&nbsp;&nbsp;<input type=radio name=screenwidth ".($CURUSER['width'] == 'narrow' ? " checked" : "")." value=narrow>窄屏<br />", 1);
...
tr_small($lang_usercp['row_funbox'],"<input type=checkbox name=showfb".($CURUSER["showfb"] == "yes" ? " checked" : "") ." value=yes>".$lang_usercp['text_show_funbox'] , 1);
		...
		case "security":
			if ($type == 'confirm') {
				...
				$resetautologin = $_POST["resetautologin"];

				if ($resetautologin == 1) {
					$sec = mksecret();
					$passhash = md5($sec . $oldpassword . $sec);
					...
					logincookie($CURUSER["id"], $passh ,1,0,$securelogin_indentity_cookie,$ssl);
					$rstautologin=1;
				}
...
EOD;
			...
			if ($type == 'save') {
				if ($CURUSER[id]==38630){
					bark($lang_usercp['permissiondenied'].goback(), 0);
					die;}
				...
				if ($resetautologin == 1)
				print("<input type=\"hidden\" name=\"resetautologin\" value=\"1\">");
			...
			if ($type == 'saved')
				print("<tr><td colspan=2 class=\"heading\" valign=\"top\" align=\"center\"><font color=red>".$lang_usercp['text_saved'].($_GET["mail"] == "1" ? $lang_usercp['std_confirmation_email_sent'] : "")." ".($_GET["resetautologin"] == "1" ? $lang_usercp['std_resetautologin'] : "")." ".($_GET["passkey"] == "1" ? $lang_usercp['std_passkey_reset'] : "")." ".($_GET["password"] == "1" ? $lang_usercp['std_password_changed'] : "")." ".($_GET["privacy"] == "1" ? $lang_usercp['std_privacy_level_updated'] : "")."</font></td></tr>\n");
			form ("security");
			...
			tr_small($lang_usercp['row_reset_autologin'],"<input type=checkbox name=resetautologin value=1 />".$lang_usercp['checkbox_reset_autologin']."<br /><font class=small>".$lang_usercp['text_reset_autologin_note']."</font>", 1);
