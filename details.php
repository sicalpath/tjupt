...
if (get_user_class () < UC_MODERATOR) {
	$addicond = " AND torrents_c.pulling_out = '0'";
}

$res = sql_query ( "SELECT torrents_c.cache_stamp, torrents_c.sp_state, torrents_c.sp_time, torrents_c.promotion_time_type, torrents_c.promotion_until, torrents_c.pos_state, torrents_c.picktype, torrents_c.category, torrents_c.url, torrents_c.small_descr, torrents_c.seeders, torrents_c.banned, torrents_c.leechers, torrents_c.info_hash, torrents_c.filename, nfo, LENGTH(torrents_c.nfo) AS nfosz, torrents_c.last_action, torrents_c.last_seed, torrents_c.needkeepseed, torrents_c.seedkeeper, torrents_c.name, torrents_c.owner, torrents_c.save_as, torrents_c.descr, torrents_c.visible, torrents_c.size, torrents_c.added, torrents_c.views, torrents_c.hits, torrents_c.times_completed, torrents_c.id, torrents_c.type, torrents_c.numfiles, torrents_c.anonymous, torrents_c.pulling_out, torrents_c.imdb_rating, categories.name AS cat_name, sources.name AS source_name, media.name AS medium_name, codecs.name AS codec_name, standards.name AS standard_name, processings.name AS processing_name, teams.name AS team_name, audiocodecs.name AS audiocodec_name, torrents_c.pulling_out AS pulling_out, torrents_c.pos_state_until as pos_state_until FROM torrents_c LEFT JOIN categories ON torrents_c.category = categories.id LEFT JOIN sources ON torrents_c.source = sources.id LEFT JOIN media ON torrents_c.medium = media.id LEFT JOIN codecs ON torrents_c.codec = codecs.id LEFT JOIN standards ON torrents_c.standard = standards.id LEFT JOIN processings ON torrents_c.processing = processings.id LEFT JOIN teams ON torrents_c.team = teams.id LEFT JOIN audiocodecs ON torrents_c.audiocodec = audiocodecs.id WHERE torrents_c.id = $id $addicond LIMIT 1" ) or sqlerr ();
