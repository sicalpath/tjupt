...
function torrenttable($res, $variant = "torrent") {
	...
	if ($torrent_uploaded===false || $torrent_seeding===false || $torrent_leeching===false || $torrent_completed===false || $torrent_incomplete===false || $torrent_leeching_rat===false || $torrent_incomplete_rat===false) {
		$res_uploaded = sql_query ( "SELECT torrents_c.id AS torrent FROM torrents_c LEFT JOIN snatched ON torrents_c.id = snatched.torrentid WHERE owner=" . ($CURUSER ["id"]) . " AND snatched.userid=" . ($CURUSER ["id"]) . " ORDER BY added DESC" ) or sqlerr ( __FILE__, __LINE__ );
		...
		$res_incomplete = sql_query ( "SELECT torrents_c.id AS torrent, torrents_c.size AS size, snatched.to_go AS to_go FROM torrents_c LEFT JOIN snatched ON torrents_c.id = snatched.torrentid WHERE snatched.finished='no' AND userid=" . ($CURUSER ["id"]) . " AND torrents_c.owner != " . ($CURUSER ["id"]) . " ORDER BY snatched.startdat DESC" ) or sqlerr ();
