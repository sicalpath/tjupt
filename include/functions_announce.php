...
function check_cheater($userid, $username, $torrentid, $uploaded, $downloaded, $anctime, $ip, $port, $seeders = 0, $leechers = 0)
{
    ...
    if ($uploaded > 1073741824 && $upspeed > (131072000)) //Uploaded more than 1 GB with uploading rate higher than 125 MByte/s. This is no doubt cheating.
    {
        $comment = "用户被系统自动禁用！";
    ...
    if (preg_match("/^172.26/", $ip)) // If is new campus with GbE
    {
        $upload_spd_warn_limit = 52428800; // 50MB/s
    }
    else // Old campus with 100Mbps
    {
        $upload_spd_warn_limit = 13107200; // 12.5MB/s
    }
    if ($uploaded > 1073741824 && $upspeed > $upload_spd_warn_limit)
    {
