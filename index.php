...
if ($showextinfo ['imdb'] == 'yes')
	...
	// if ($_SERVER['SERVER_NAME']=="202.113.13.170")header("Location:
// http://pt.tju.edu.cn/");
...
if (! $Cache->get_page ()) {
	...
	if (mysql_num_rows ( $res ) > 0) {
		$Cache->add_whole_row ();
		print ("<table width=\"100%\" ><tr><td class=\"text\" valign=\"top\" ><div style=\"margin-left: 16pt;\">\n") ;
		$Cache->end_whole_row ();
		$news_flag = 0;
...
// ------------- end: recent news ------------------//
?>
<td width="25%" valign="top"><iframe height="500" class="share_self"
		frameborder="0" scrolling="no"
		src="http://widget.weibo.com/weiboshow/index.php?width=0&height=500&fansRow=2&ptype=1&speed=0&skin=2&isTitle=0&noborder=1&isWeibo=1&isFans=0&uid=2308910882&verifier=fe26ef74"></iframe></td>
</tr>
</table>
<?php
// ------------- start: shoutbox ------------------//
if ($showshoutbox_main == "yes") {
	?>
<h2><?php echo $lang_index['text_shoutbox'] ?> - <font class="small"><?php echo $lang_index['text_auto_refresh_after']?></font><font
		class='striking' id="countdown"></font><font class="small"><?php echo $lang_index['text_seconds']?></font>
</h2>
<?php
...
if ($showlastxtorrents_main == "yes") {
	$result = sql_query ( "SELECT * FROM torrents where visible='yes' AND torrents.category < 412 ORDER BY added DESC LIMIT 5" ) or sqlerr ( __FILE__, __LINE__ );
...
/*
 * if ($showlastxforumposts_main == "yes" && $CURUSER) { $res =
 * sql_query("SELECT posts.id AS pid, posts.userid AS userpost, posts.added,
 * topics.id AS tid, topics.subject, topics.forumid, topics.views, forums.name
 * FROM posts, topics, forums WHERE posts.topicid = topics.id AND topics.forumid
 * = forums.id AND minclassread <=" . sqlesc(get_user_class()) . " ORDER BY
 * posts.id DESC LIMIT 5") or sqlerr(__FILE__,__LINE__); if(mysql_num_rows($res)
 * != 0) { print("<h2>".$lang_index['text_last_five_posts']."</h2>");
 * print("<table width=\"100%\" border=\"1\" cellspacing=\"0\"
 * cellpadding=\"5\"><tr><td class=\"colhead\" width=\"100%\"
 * align=\"left\">".$lang_index['col_topic_title']."</td><td class=\"colhead\"
 * align=\"center\">".$lang_index['col_view']."</td><td class=\"colhead\"
 * align=\"center\">".$lang_index['col_author']."</td><td class=\"colhead\"
 * align=\"left\">".$lang_index['col_posted_at']."</td></tr>"); while ($postsx =
 * mysql_fetch_assoc($res)) { print("<tr><td><a
 * href=\"forums.php?action=viewtopic&amp;topicid=".$postsx["tid"]."&amp;page=p".$postsx["pid"]."#pid".$postsx["pid"]."\"><b>".htmlspecialchars($postsx["subject"])."</b></a><br
 * />".$lang_index['text_in']."<a
 * href=\"forums.php?action=viewforum&amp;forumid=".$postsx["forumid"]."\">".htmlspecialchars($postsx["name"])."</a></td><td
 * align=\"center\">".$postsx["views"]."</td><td align=\"center\">" .
 * get_username($postsx["userpost"])
 * ."</td><td>".gettime($postsx["added"])."</td></tr>"); } print("</table>"); }
 * }
 */
...
if ($CURUSER && $showpolls_main == "yes") {
	...
	print ("<h2><a name=\"poll\"></a>" . $lang_index ['text_polls']) ;
...
<table width="100%">
	<tr>
		<td class="text" align="center">
			<table width="60%" class="main" border="1" cellspacing="0"
				cellpadding="10">
<?php
...
<?php
	...
	if (! $Cache->get_page ()) {
		...
		$totalbonus = number_format ( get_single_value ( "users WHERE enabled='yes'", "sum(seedbonus) " ) );
		$totalinvites = number_format ( get_single_value ( "users WHERE enabled='yes'", "sum(invites)" ) );
...
<?php
		...
		?>
				...
				<tr>
<?php
		twotd ( $lang_index ['row_total_bonus'], $totalbonus );
		twotd ( $lang_index ['row_total_invites'], $totalinvites );
		?>
</tr>
...
<h2><?php echo $lang_index['text_disclaimer'] ?></h2>
			<table width="100%">
				<tr>
					<td class="text">
  <?php echo $lang_index['text_disclaimer_content'] ?></td>
				</tr>
			</table>
<?php
// ------------- end: disclaimer ------------------//
...
if (! $Cache->get_page ()) {
	...
	if (mysql_num_rows ( $res ) > 0) {
		...
		print ("<table width=\"100%\"><tr><td class=\"text\" id=\"addad\">" . trim ( $links ) . "</td></tr></table>") ;
	}
	$Cache->end_whole_row ();
	$Cache->cache_page ();
}
...
?>
<table width="100%" class="main" border="0" cellspacing="0"
				cellpadding="0">
				<tr>
					<td class="embedded">
						<div align="center">
							<br />
							<font class="medium"><?php echo $lang_index['text_browser_note'] ?></font>
						</div>
