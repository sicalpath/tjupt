...
(function(){
var friendsData = [{username:"最多可以@十个用户",userid:"用户名长度小于5"}];
// 上面是数据
var config = {
		boxID:"autoTalkBox",
...
var DS = {
	inquiry:function(data, str, num){
		...
		var phpdata=ajax.posts('at_user.php','str='+encodeURI(str));
		if(phpdata == '') return friendsData.slice(0, num);
		eval("phpdataajax = " + phpdata + ";") ;
		var reg = new RegExp(str, 'i');
		var i = 0;
		//var dataUserName = {};
		var sd = [];

		while(sd.length < num && i < phpdataajax.length){
			if(reg.test(phpdataajax[i]['username']))
			{
				sd.push(phpdataajax[i]);
...
var AutoTips = function(A){
	...
	_this.start = function(){
  	...
  	_this.keyupFn = function(e){
		...
		var chars = val.match(/([@\[]?)@(\S+)$|([@\[]?)@$/);
		if(chars == null||chars[1]||chars[3]) return _this.hide();
	...
	_this.showList = function(char){
		...
		for(var i=0; i<len; i++){
			...
			h += html.replace(/\$ACCOUNT\$|\$NAME\$/g,data[i]['username']).replace('$SACCOUNT$',data[i]['userid']).replace('$ID$',"@"+data[i]['username']+"");
		}
