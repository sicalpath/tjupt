...
if (! in_array ( $action, $allowed_actions ))
	...
	function reqmenu($selected = "request") {
		global $title;
		...
		print ("<li" . ($selected == "request" ? " class=selected" : "") . "><a href=\"?action=list\">求 种</a></li>") ;
		
		print ("<li" . ($selected == "reseed" ? " class=selected" : "") . "><a href=\"?action=reseed\">补 种</a></li>") ;
		
		print ("<li" . ($selected == "keepseed" ? " class=selected" : "") . "><a href=\"?action=keepseed\">保 种</a></li>") ;
		
		print ("<li" . ($selected == "needtran" ? " class=selected" : "") . "><a href=\"?action=needtran\">中 转</a></li>") ;
	...
	switch ($action) {
		...
		case "list" :
			
			{
				...
				if ($rownumber [0] == 0)
					stderr ( "没有求种", "没有符合条件的求种项目，<a href=viewrequests.php?action=new>点击这里增加新求种</a>", 0, 0, 0, 0 );
					...
					print ("<h1 align=center><a href=viewrequests.php?action=new><input type=\"button\" value=\"" . $lang_req ['head_req'] . "\" onclick=\"window.location='viewrequests.php?action=new';\" style=\"font-weight: bold\"/></a><a href=forums.php?action=viewtopic&forumid=5&topicid=9060><input type=\"button\" value=\"规  则\" onclick=\"window.location='forums.php?action=viewtopic&forumid=5&topicid=9060';\" style=\"font-weight: bold\"/></a></h1>") ;
					
					print ($pagertop) ;
					
					// print("<table width=940 border=1 cellspacing=0
...

					{
						...
						print ("<form action=\"viewrequests.php\" method=\"GET\" onsubmit=\"return confirm('确认操作？');\">\n") ;
					...
					"<td class=colhead width=3% align=center><a href=?" . $orderlink . "sort=1&type=" . ($column == "catid" && $ascdesc == "ASC," ? "desc" : "asc") . ">类型</a></td>" . 

					"<td class=colhead width=40% align=left><a href=?" . $orderlink . "sort=2&type=" . ($column == "name" && $ascdesc == "ASC," ? "desc" : "asc") . ">" . $lang_req ['name'] . "</a></td>" . 

					"<td class=colhead width=5% align=center><a href=?" . $orderlink . "sort=3&type=" . ($column == "amount" && $ascdesc == "DESC," ? "asc" : "desc") . ">" . $lang_req ['new_amount'] . "</a></td>" . 

					"<td class=colhead width=5% align=center><a href=?" . $orderlink . "sort=4&type=" . ($column == "ori_amount" && $ascdesc == "DESC," ? "asc" : "desc") . ">" . $lang_req ['ori_amount'] . "</a></td>" . 

					"<td class=colhead width=7% align=center><a href=?" . $orderlink . "sort=5&type=" . ($column == "userid" && $ascdesc == "ASC," ? "desc" : "asc") . ">" . $lang_req ['addedby'] . "</a></td>" . 

					"<td class=colhead width=4% align=center><a href=?" . $orderlink . "sort=6&type=" . ($column == "comments" && $ascdesc == "DESC," ? "asc" : "desc") . ">评论</a></td>" . 

					"<td class=colhead width=4% align=center ><font color=black>应求</font></a></td>" . 

					"<td class=colhead width=9% align=center><a href=?" . $orderlink . "sort=7&type=" . ($column == "added" && $ascdesc == "DESC," ? "asc" : "desc") . ">" . $lang_req ['addedtime'] . "</a></td>" . 

					"<td class=colhead width=8% align=center><a href=?" . $orderlink . "sort=9&type=" . ($column == "resetdate" && $ascdesc == "DESC," ? "asc" : "desc") . ">最后悬赏</a></td>" . 

					"<td class=colhead width=8% align=center><a href=?" . $orderlink . "sort=8&type=" . ($column == "finish" && $ascdesc == "DESC," ? "asc" : "desc") . ">" . $lang_req ['state'] . "</a></td>\n" . 
...

					{
						...
						$count = $Cache->get_value ( 'resreq_count_reqid_' . $row ["id"] );
						if ($count === false) {
							$count = mysql_fetch_assoc ( sql_query ( "SELECT count(*) FROM resreq WHERE reqid=" . $row ["id"] ) );
							$count = $count ["count(*)"];
							$Cache->cache_value ( 'resreq_count_reqid_' . $row ["id"], $count, 900 );
						}
						...
						"<td align=center><font color=#ff0000><b>" . $row ['amount'] . "</b></font></td>" . 

						"<td align=center>" . $row ['ori_amount'] . "</td>" . 

						"<td align=center>" . get_username ( $row ['userid'] ) . "</td>" . 

						"<td align=center>" . $row ['comments'] . "</td>" . 

						"<td align=center>" . $count . "</td>" . 

						"<td align=center>" . gettime ( $row ['added'], true, false ) . "</td>" . 
				...
				print ("<br><a href=viewrequests.php?finished=all>查看所有</a> <a href=viewrequests.php?finished=yes>查看已解决</a> <a href=viewrequests.php?finished=no>查看未解决</a> <a href=viewrequests.php?finished=cancel>查看已关闭</a>\n") ;
		...
		case "view" :
			
			{
				...
				if (is_numeric ( $_GET ["id"] )) {
					...
					tr ( "悬赏", $lang_req ['new_amount'] . $arr ['amount'] . $lang_req ['ori_amount_is_1'] . $arr ["ori_amount"] . $lang_req ['ori_amount_is_2'] . "\n", 1 );
					...
					if ($arr ["finish"] == "no")
						tr ( "追加悬赏", "<form action=viewrequests.php method=post> <input type=hidden name=action value=addamount><input type=hidden name=reqid value=" . $arr ["id"] . "><input size=6 name=amount value=1000 ><input type=submit value=提交 > 追加悬赏每次将扣减25个魔力值作为手续费" . $amountadder . "</form>", 1 );
					...
					if (mysql_num_rows ( $res ) == 0)
...

						{
							...
							$ress .= (get_user_class () >= 13 ? " [<a href=viewrequests.php?action=delres&id=" . $row ["id"] . " >X</a>]" : "") . 
...
function quick_reply_to(username)
{
    ...
    parent.document.getElementById("quickreplytext").value = "@" + username + " "+parent.document.getElementById("quickreplytext").value;
					...
					print ("<a name='quickreply' id='quickreply'> </a><table style='border:1px solid #000000;'><tr>" . 

					"<td class=\"text\" align=\"center\"><b>快速留言</b><br /><br />" . 

					"<form id=\"compose\" name=\"comment\" method=\"post\" action=\"comment.php?action=add&amp;type=request\" onsubmit=\"return postvalid(this);\">" . 
		...
		") ;
					...
					foreach ( $cats as $cat )
						...
						$select .= "<option value=\"" . $cat ["id"] . "\" " . ($arr ["catid"] == $cat ["id"] ? "selected " : "") . ">" . $cat ["name"] . "</option>";
					...
					stderr ( "出错了！！！", "你没有该权限！！！<a href=viewrequests.php?action=view&id=" . $_GET ["id"] . ">点击这里返回</a>", 0 );
			}
		...
		case "new" :
			
			{
...

				{
					...
					stderr ( "出错了！！！", "你没有该权限！！！<a href=viewrequests.php>点击这里返回</a>", 0 );
			}
		...
		case "takeadded" :
			
			{
...

				{
					...
					stderr ( "成功", "新增求种成功，<a href=viewrequests.php>点击这里返回</a>", 0 );
				} 
		...
		case "takeedit" :
			
			{
				...
				if (mysql_num_rows ( $res ) == 0)
					stderr ( "出错了！", "该求种已被删除！<a href=viewrequests.php>点击这里返回</a>", 0 );
...

				{
					...
					stderr ( "出错了！！！", "你没有该权限！！！<a href=viewrequests.php?action=view&id=" . $_POST ["reqid"] . ">点击这里返回</a>", 0 );
	...
	<input type=hidden name=reqid value=\"" . $_GET ["id"] . "\" />
	请输入种子的id:<br/>http://pt.tju.edu.cn/details.php?id=<input type=text name=torrentid size=11/><br/>
	<input type=submit value=提交 >&nbsp;&nbsp;&nbsp;&nbsp;<input type=button onclick=\"location.href='javascript:history.go(-1)'\" value=\"返回\" ></form>", 0 );
		...
		case "takeres" :
			
			{
				...
				if ($arr ["finish"] != "no")
					stderr ( "出错了！", "该求种已完成或撤销！<a href=viewrequests.php?action=view&id=" . $_POST ["reqid"] . ">点击这里返回</a>", 0 );
				
				if (! is_numeric ( $_POST ["torrentid"] ))
					stderr ( "出错了！！！", "种子ID必须为数字！<a href=viewrequests.php?action=res&id=" . $_POST ["reqid"] . ">点击这里返回</a>", 0 );
				
				$res = sql_query ( "SELECT * FROM torrents WHERE id =" . sqlesc ( $_POST ["torrentid"] ) . "" ) or sqlerr ( __FILE__, __LINE__ );
				
				if (mysql_num_rows ( $res ) == 0)
					stderr ( "出错了！", "该种子不存在！<a href=viewrequests.php?action=res&id=" . $_POST ["reqid"] . ">点击这里返回</a>", 0 );
				...
				sql_query ( "INSERT INTO resreq (reqid , torrentid , submitted_by ) VALUES ( " . sqlesc ( $_POST ["reqid"] ) . " , " . sqlesc ( $_POST ["torrentid"] ) . " , ". $CURUSER ["id"]. ")" );
				
				stderr ( "成功", "应求成功，<a href=viewrequests.php?action=view&id=" . $_POST ["reqid"] . ">点击这里返回</a>", 0, 1, 1, 0 );
				
				$Cache->delete_value ( 'resreq_count_reqid_' . $arr ["id"] );
				...
				$notifs = sqlesc ( $CURUSER ["username"] . "应求了你的求种" . "[url=viewrequests.php?action=view&id=" . $arr ["id"] . "]" . $arr ["name"] . "[/url]" );
		...
		case "addamount" :
			
			{
				...
				stderr ( "成功", "追加悬赏成功，<a href=viewrequests.php?action=view&id=" . $_POST ["reqid"] . ">点击这里返回</a>", false );
...

				{
					...
					if ($_POST ["reasontype"]) {
						...
						if (! is_int ( $rt ) || $rt < 1 || $rt > 5)
							...
							stderr ( "出错了！", "删除理由" . $rt . "不成立，<a href=viewrequests.php?action=delete&id=" . $_GET ["id"] . " >请返回删除页面重选</a>", false );
...

						{
							...
							if (! $r [2])
								...
								stderr ( "出错了！", "删除理由未填写，<a href=viewrequests.php?action=delete&id=" . $_GET ["id"] . " >请返回删除页面重填</a>", false );
						...
						stderr ( "成功", "删除求种成功，<a href=viewrequests.php>点击这里返回</a>", false );
					} 

					else {
						
						print ("<form action=\"viewrequests.php?action=delete&id=" . $_GET ["id"] . "\" method=\"post\" >") ;
						
						stderr ( "删除求种-理由：", 

						"<table align=\"center\" border=\"1\" cellspacing=\"0\" cellpadding=\"5\">\n" . 

						"<tr><td class=\"rowhead\" valign=\"top\" align=\"right\"><input name=\"reasontype\" type=\"radio\" value=\"1\" />&nbsp;成功</td>" . 

						"<td class=\"rowfollow\" valign=\"middle\" align=\"left\">已经有用户成功发布该资源</td></tr>" . 

						"<td class=\"rowhead\" valign=\"top\" align=\"right\"><input name=\"reasontype\" type=\"radio\" value=\"2\" />&nbsp;过期" . "</td>" . 

						"<td class=\"rowfollow\" valign=\"middle\" align=\"left\">长时间无人应求</td></tr>" . 

						"<td class=\"rowhead\" valign=\"top\" align=\"right\"><input name=\"reasontype\" type=\"radio\" value=\"3\" />&nbsp;重复</td>" . 

						"<td class=\"rowfollow\" ><input type=\"text\" style=\"width: 200px\" name=\"reason[]\" /></td></tr>" . 

						"<td class=\"rowhead\" valign=\"top\" align=\"right\"><input name=\"reasontype\" type=\"radio\" value=\"4\" />&nbsp;" . "格式错</td>" . 

						"<td class=\"rowfollow\" valign=\"top\" align=\"left\"><input type=\"text\" style=\"width: 200px\" name=\"reason[]\" /></td></tr>" . 

						"<td class=\"rowhead \" valign=\"top\" align=\"right\"><input name=\"reasontype\" type=\"radio\" value=\"5\" checked=\"checked\" />&nbsp;" . "其他</td>" . 

						"<td class=\"rowfollow\" valign=\"top\" align=\"left\"><input type=\"text\" style=\"width: 200px\" name=\"reason[]\" />(必填)</td></tr>" . 

						"<tr><td class=\"toolbox\" colspan=\"2\" align=\"center\"><input type=\"submit\" style='height: 25px' value=\"删除\" />&nbsp;&nbsp;&nbsp;&nbsp;<input type=button style='height: 25px' onclick=\"location.href='javascript:history.go(-1)'\" value=\"返回\" ></td></tr></table>\n" . 
		...
		case "fastdel" :
			
			{
...

				{
...

					{
...

						{
							...
							$notifs = sqlesc ( "管理员" . $CURUSER ["username"] . "撤销了你的求种 [url=viewrequests.php?action=view&id=" . $arr ["id"] . "]" . $arr ["name"] . "[/url],赏金已经退回到相关账户中。" );
				...
				stderr ( "成功", "操作成功，<a href=\"viewrequests.php\">点击这里返回</a>", false );
		...
		case "delres" :
			
			{
...

				{
					...
					$Cache->delete_value ( 'resreq_count_reqid_' . $arr ["reqid"] );
					
					header ( "Location: viewrequests.php?action=view&id=" . $arr ["reqid"] );
				} 
	...
	有应求的种子不允许自行撤销求种，<a href=\"JAVAscript:document.contact_cancel.submit();\">点击这里联系管理组</a>（请勿重复点击！），<a href=viewrequests.php?action=view&id=" . $_GET ["id"] . ">点击这里返回</a>", false );
				...
				if ($arr ['added'] > date ( "Y-m-d H:i:s", time () - 14 * 86400 ) && get_user_class () < 13)
					stderr ( "出错了！", "发布求种2周后才允许自行撤销！！<a href=viewrequests.php?action=view&id=" . $_GET ["id"] . ">点击这里返回</a>", false );
				
				{
					...
					stderr ( "成功", "撤销求种成功，<a href=viewrequests.php>点击这里返回</a>", false );
				}
		...
		case "confirm" :
			
			{
				...
				if (empty ( $_POST ["torrentid"] ))
					...
					stderr ( "出错了！", "该应求已经确认！<a href=viewrequests.php?action=view&id=" . $_POST ["id"] . ">点击这里返回</a>", false );
...

				{
					...
					stderr ( "成功", "确认成功，<a href=viewrequests.php?action=view&id=" . $_POST ["id"] . ">点击这里返回</a>", false );
				}
		...
		case "reseed" :
			
			{
				...
				$sql = "SELECT count(*) FROM torrents WHERE $cat last_seed < last_reseed";
...

				{
					...
					$addparam = $oldlink . $pagerlink;
				} 

				else
					stderr ( "暂时不需要补种", "目前没有收到任何补种请求，请过一会再来看看。", 0, 0 );
				
				list ( $pagertop, $pagerbottom, $limit ) = pager ( $torrentsperpage, $count, "?" . $addparam );
				
				$sql_ = "SELECT * FROM torrents WHERE $cat last_seed < last_reseed ORDER BY $column $ascdesc last_reseed DESC $limit";
				
				$res_ = sql_query ( $sql_ ) or die ( mysql_error () );
				
				if ($count) {
					
					print ($pagertop) ;
					
					torrenttable ( $res_, "torrents" );
		...
		case "keepseed" :
			
			{
				...
				$sql = "SELECT count(*) FROM torrents WHERE needkeepseed = 'yes' AND seedkeeper < 2";
...

				{
					...
					stderr ( "暂时不需要保种", "目前所有需要保种资源已经分配完毕，请过一会再来看看。", 0, 0 );
				
				list ( $pagertop, $pagerbottom, $limit ) = pager ( $torrentsperpage, $count, "?" . $addparam );
				
				$sql = "SELECT * FROM torrents WHERE $cat needkeepseed = 'yes' AND seedkeeper < 2 ORDER BY $column $ascdesc last_reseed DESC $limit";
