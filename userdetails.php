...
$arr3 = mysql_fetch_row ( $res );
$forumposts = $arr3 [0];
$res = sql_query ( "SELECT COUNT(*) FROM attachments WHERE userid=" . $user [id] ) or sqlerr ();
$arr3 = mysql_fetch_row ( $res );
$attachments = $arr3 [0];
...
if ($CURUSER[id] == $user[id] || get_user_class() >= $cruprfmanage_class)
	print("<h2>".$lang_userdetails['text_flush_ghost_torrents']."<a class=\"altlink\" href=\"takeflush.php?id=".$id."\">".$lang_userdetails['text_here']."</a>（这可不是点着玩的，有可能造成一段时间内上传量和魔力值无法计算，请谨慎使用。）</h2>\n");
?>
...
*/
	if (isset($CURUSER) && $CURUSER[id] != $user[id])
	{
		//$user_snatched = sql_query ( "SELECT * FROM snatched WHERE userid = $CURUSER[id] AND finished='yes'" ) or sqlerr ( __FILE__, __LINE__ );
		$user_snatched = sql_query ( "SELECT torrents.id AS torrentid FROM torrents LEFT JOIN snatched ON torrents.id = snatched.torrentid WHERE snatched.userid=$CURUSER[id] AND (torrents.owner=$CURUSER[id] OR snatched.finished='yes') ORDER BY added DESC" ) or sqlerr ( __FILE__, __LINE__ );
		
		if (mysql_num_rows ( $user_snatched ) == 0)
			$compatibility_info = $lang_userdetails ['text_unknown'];
		else {
			global $Cache;
			$tjusim = $Cache->get_value ( "user_similarity_jaccard_" . $CURUSER [id] . "_" . $user [id] );
			if ($tjusim === false) {
				$curuser_snatched_tids = array ();
				while ( $user_snatched_row = mysql_fetch_array ( $user_snatched ) ) {
					$curuser_snatched_tids [] = $user_snatched_row ["torrentid"];
				}
				
				$viewuser_snatched_tids = array ();
// 				$viewuser_snatched = sql_query ( "SELECT * FROM snatched WHERE userid = $user[id] AND finished='yes'" ) or sqlerr ( __FILE__, __LINE__ );
				$viewuser_snatched = sql_query ( "SELECT torrents.id AS torrentid FROM torrents LEFT JOIN snatched ON torrents.id = snatched.torrentid WHERE snatched.userid=$user[id] AND (torrents.owner=$user[id] OR snatched.finished='yes') ORDER BY added DESC" ) or sqlerr ( __FILE__, __LINE__ );
				while ( $viewuser_snatched_row = mysql_fetch_array ( $viewuser_snatched ) ) {
					$viewuser_snatched_tids [] = $viewuser_snatched_row ["torrentid"];
				}
				
				$tjusim = get_similarity_simple ( $curuser_snatched_tids, $viewuser_snatched_tids );
				if ($tjusim > 1) {
					$tjusim = 1;
				} elseif ($tjusim < 0) {
					$tjusim = 0;
				...
				$Cache->cache_value ( "user_similarity_jaccard_" . $CURUSER [id] . "_" . $user [id], $tjusim, 1800 );
			}
			if ($tjusim >= 0.8) {
				$compatibility_info = $lang_userdetails ['text_super'];
				$bar_url = "pic/loadbargreen.gif";
			} elseif ($tjusim >= 0.6 && $tjusim < 8) {
				$compatibility_info = $lang_userdetails ['text_very_high'];
				$bar_url = "pic/loadbargreen.gif";
			} elseif ($tjusim >= 0.4 && $tjusim < 0.6) {
				$compatibility_info = $lang_userdetails ['text_high'];
				$bar_url = "pic/loadbargreen.gif";
			} elseif ($tjusim >= 0.2 && $tjusim < 0.4) {
				$compatibility_info = $lang_userdetails ['text_medium'];
				$bar_url = "pic/loadbaryellow.gif";
			} elseif ($tjusim > 0.05 && $tjusim < 0.2) {
				$compatibility_info = $lang_userdetails ['text_low'];
				$bar_url = "pic/loadbarred.gif";
			} else {
				$compatibility_info = $lang_userdetails ['text_very_low'];
			...
			$width = $tjusim * 400;
			$compatibility_info = "<table align=left border=0 width=400><tr><td style='padding: 0px; background-image: url(pic/loadbarbg.gif); background-repeat: repeat-x; width: 400px' title='" . number_format ( $tjusim * 100, 2 ) . "%'><img align=left height=15 width=" . $width . " src=\"" . $bar_url . "\" alt='" . number_format ( $tjusim * 100, 2 ) . "%'></td><td align=right class=embedded><strong>&nbsp;&nbsp;&nbsp;<nobr>" . $compatibility_info . "</nobr> </strong></td></tr></table>";
		}
		print ("<tr><td class=rowhead width=13%>" . $lang_userdetails ['row_compatibility'] . "</td><td class=rowfollow align=left width=87%>" . $compatibility_info . "</td></tr>\n") ;
	}
	...
	if ($user["invites"] <= 0)
	tr_small($lang_userdetails['row_invitation'], "<a href=\"invite.php?id=".$user[id]."\" title=\"".$lang_userdetails['text_no_invitation']."\">".$lang_userdetails['text_no_invitation']."</a>", 1);
	else
...
if (get_user_class() >= $userprofile_class ||  $user["id"] == $CURUSER["id"])
{
	...
	tr_small($lang_userdetails['row_ip_address'], $user[ip].$school.$locationinfo, 1);
}

//$res = sql_query("SELECT agent, peer_id, port FROM peers WHERE userid = $user[id] GROUP BY agent") or sqlerr();
$res = sql_query("SELECT agent, peer_id, ipv4, ipv6, port FROM peers WHERE userid = $user[id] GROUP BY ipv4, ipv6, port") or sqlerr();
//$res = sql_query("SELECT agent, peer_id, MAX(ipv4) AS ipv4,MAX(ipv6) AS ipv6, port FROM peers WHERE userid = $user[id] GROUP by port, (ipv4 OR ipv6)") or sqlerr();
...
if (mysql_num_rows($res) > 0)
{
	...
	{
		$clientselect .= ($first == true ? "" : "<br />") . get_agent($arr["peer_id"], $arr["agent"]);
		$first = false;

		if (get_user_class() >= $userprofile_class ||  $user["id"] == $CURUSER["id"])
		$clientselect .= " (" .$arr["ipv4"].($arr["ipv6"]?($arr["ipv4"]?":".$arr["port"].", [".$arr["ipv6"]."]":"[".$arr["ipv6"]."]"):""). ":" . $arr["port"] . ")";
	}
}
if ($clientselect)
...
tr_small($lang_userdetails['row_school'], $schoollist[$user["school"]], 1);
...
tr_small($lang_userdetails['row_class'], "<img alt=\"".get_user_class_name($user["class"],false,false,true)."\" title=\"".get_user_class_name($user["class"],false,false,true)."\" src=\"".$uclass."\" /> ".(($user["id"] == $CURUSER["id"] || get_user_class() >= $viewhistory_class) ? ($user[title]!=="" ? "&nbsp;&nbsp;自定义头衔：".htmlspecialchars(trim($user["title"]))."" :  "") . "<a href=\"myhistory.php?type=account&amp;id=".$id."\">&nbsp;&nbsp;&nbsp;<b>查看账户历史</b></a>" : ($user[title]!=="" ? "&nbsp;&nbsp;自定义头衔：".htmlspecialchars(trim($user["title"]))."" :  "")), 1);
...
tr_small($lang_userdetails['row_forum_attach'], ($attachments && ($user["id"] == $CURUSER["id"] || get_user_class() >= $viewhistory_class) ? "<a href=\"userhistory.php?action=viewattach&amp;id=".$id."\" title=\"".$lang_userdetails['link_view_attach']."\">".$attachments."</a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href=\"userhistory.php?action=viewattach&amp;id=".$id."&amp;unused=1\"><font title=\"查看未使用\" color=red><b>查看未使用</b></font></a>" : $attachments), 1);

if ($user["id"] == $CURUSER["id"] || get_user_class() >= $viewhistory_class)
tr_small($lang_userdetails['row_karma_points'], ($user["id"] == $CURUSER["id"] || get_user_class() >= $viewhistory_class) ?htmlspecialchars($user[seedbonus]). "<a href=\"myhistory.php?type=bonus&amp;id=".$id."\">&nbsp;&nbsp;&nbsp;<b>查看魔力值历史</b></a>" : htmlspecialchars($user[seedbonus]), 1);
...
if ($CURUSER["id"] == $user["id"]&&$_SERVER['HTTP_USERNAME']!=""){
	...
	if (mysql_num_rows($res) > 0)
		tr_small($lang_userdetails['carsiuser'],$lang_userdetails['carsiname'].$schools[school].$lang_userdetails['carsiinstitution'].$username.$lang_userdetails['alreadybinded'].$unbindbutton,1);
	else
		tr_small($lang_userdetails['carsiuser'],$lang_userdetails['carsiname'].$schools[school].$lang_userdetails['carsiinstitution'].$username.$lang_userdetails['carsibind'].$bindbutton, 1);	
}
...
{
	$uclass = get_user_class_image($user["class"]);
	tr_small($lang_userdetails['row_class'], "<img alt=\"".get_user_class_name($user["class"],false,false,true)."\" title=\"".get_user_class_name($user["class"],false,false,true)."\" src=\"".$uclass."\" /> ".($user[title]!=="" ? "&nbsp;&nbsp;自定义头衔：".htmlspecialchars(trim($user["title"]))."" :  ""), 1);	
...
if (get_user_class() >= $prfmanage_class && $user["class"] < get_user_class())
{
	...
	tr('颜色', "<input type=\"text\" size=\"60\" name=\"color\" value=\"" . htmlspecialchars(trim($user[color])) . "\" />", 1);
	...
	$designer = htmlspecialchars($user["designer"]);
	$designerfor = htmlspecialchars($user["designerfor"]);
	...
	tr('美工', "<input type=\"radio\" name=\"designer\" value=\"yes\"" .($user["designer"] == "yes" ? " checked=\"checked\"" : "")." />".$lang_userdetails['radio_yes']." <input type=\"radio\" name=\"designer\" value=\"no\"" .($user["designer"] == "no" ? " checked=\"checked\"" : "")." />".$lang_userdetails['radio_no'], 1);
	tr('美工职责', "<textarea cols=\"60\" rows=\"6\" name=\"designerfor\">".$designerfor."</textarea>", 1);
	...
	print("</table></td></tr>");
	tr($lang_userdetails['row_enabled'], "<input name=\"enabled\" value=\"yes\" type=\"radio\"" . ($enabled ? " checked=\"checked\"" : "") . " />".$lang_userdetails['radio_yes']."<input name=\"enabled\" value=\"no\" type=\"radio\"" . (!$enabled ? " checked=\"checked\"" : "") . " />".$lang_userdetails['radio_no']."&nbsp; &nbsp; &nbsp; &nbsp; (如需填写封禁理由，理由填入警告系统中)", 1);
	...
	tr($lang_userdetails['row_download_possible'], "<input type=\"radio\" name=\"downloadpos\" value=\"yes\"" .($user["downloadpos"]=="yes" ? " checked=\"checked\"" : "") . " />".$lang_userdetails['radio_yes']."<input type=\"radio\" name=\"downloadpos\" value=\"no\"" .($user["downloadpos"]=="no" ? " checked=\"checked\"" : "") . " />".$lang_userdetails['radio_no']."&nbsp; &nbsp; &nbsp; &nbsp; (因违规封禁，需将本项置为“否”)", 1);
	...
	if (get_user_class() >= $cruprfmanage_class)
	{
		begin_frame($lang_userdetails['text_delete_user']);
		print("<form method=\"post\" action=\"delacctadmin.php\" name=\"deluser\">
		<input name=\"userid\" size=\"10\" type=\"hidden\" value=\"". $user["id"] ."\" />
		理由：<input name=\"reason\" style=\"width:450px\"  value=\"".""."\" />
		<input name=\"delenable\" type=\"checkbox\" onclick=\"if (this.checked) {enabledel('".$lang_userdetails['js_delete_user_note']."');}else{disabledel();}\" /><input name=\"submit\" type=\"submit\" value=\"".$lang_userdetails['submit_delete']."\" disabled=\"disabled\" />
		</form>");
		end_frame();
	}
}
