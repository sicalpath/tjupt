		...
		case 'new' :
			{
				...
				$title = $lang_forums ['text_new_topic_in'] . " <a href=\"" . htmlspecialchars ( "forums.php?action=viewforum&forumid=" . $id ) . "\">" . htmlspecialchars ( $forumname ) . "</a> " . $lang_forums ['text_forum'];
				$hassubject = true;
				break;
			}
		case 'reply' :
			{
				...
				$title = $lang_forums ['text_reply_to_topic'] . " <a href=\"" . htmlspecialchars ( "forums.php?action=viewtopic&topicid=" . $id ) . "\">" . htmlspecialchars ( $topicname ) . "</a> ";
				break;
			}
		case 'quote' :
			{
				...
				$title = $lang_forums ['text_reply_to_topic'] . " <a href=\"" . htmlspecialchars ( "forums.php?action=viewtopic&topicid=" . $topicid ) . "\">" . htmlspecialchars ( $topicname ) . "</a> ";
				...
				$body = "[quote=" . htmlspecialchars ( $arr ["username"] ) . "]" . htmlspecialchars ( unesc ( $arr ["body"] ) ) . "[/quote] @" . htmlspecialchars ( $arr ["username"] ) . " ";
				$id = $topicid;
				$type = 'reply';
				break;
			}
...
if (! $schoollist = $Cache->get_value ( 'schools_list' )) {
	$res = sql_query ( "SELECT * FROM schools WHERE 1" ) or sqlerr ( __FILE__, __LINE__ );
	while ( $row = mysql_fetch_array ( $res ) )
		$schoollist [$row ["id"]] = $row ["name"];
	$Cache->cache_value ( 'schools_list', $schoollist, 86400 );
}
...
// -------- Action: Multi jobs (moderator only)
if ($action == "multijob") {
	$forumid = 0 + $_GET ['forumid'];
	check_whether_exist ( $forumid, 'forum' );
	if (! is_forum_moderator ( $forumid, 'forum' ) && get_user_class () < $postmanage_class) {
		echo 'Permission denied.';
		die ();
	}
	foreach ( $_POST ['tids'] as $topicid ) {
		$res1 = sql_query ( "SELECT forumid, userid FROM topics WHERE id=" . sqlesc ( $topicid ) . " LIMIT 1" ) or sqlerr ( __FILE__, __LINE__ );
		$row1 = mysql_fetch_array ( $res1 );
		if (! $row1) {
			continue;
		}
		if ($row1 ['forumid'] != $forumid) {
			continue;
		}
		$userid = $row1 ['userid'];
		switch ($_POST ['job']) {
...
if ($action == "post") {
	...
	if ($type == 'edit') {
		...
		// PM被@的用户
		$msg = '请点击 [b][url=forums.php?action=viewtopic&topicid=' . $topicid . '&page=p' . $postid . '#pid' . $postid . ']' . $subject . '[/url][/b] 查看。';
		pm_at_users ( $body, $msg, "topic" );
	}
	
	// ------ All done, redirect user to the post
	
	$headerstr = "Location: forums.php?action=viewtopic&topicid=$topicid";
...
if ($action == "viewtopic") {
	...
	for($i = 0; $i < $pages; ++ $i) {
		...
		$dotted = 0;
		if ($i != $page)
			$pagerarr [] .= "<a href=\"" . htmlspecialchars ( "forums.php?" . $addparam . "&page=" . $i ) . "\"><b>" . ($i + 1) . "</b></a>\n";
		else
	...
	if ($page == 0)
		...
		$pager = "<a href=\"" . htmlspecialchars ( "forums.php?" . $addparam . "&page=" . ($page - 1) ) . "\"><b>&lt;&lt;" . $lang_forums ['text_prev'] . "</b></a>";
	$pager .= "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;";
	if ($page == $pages - 1)
		$pager .= "<font class=\"gray\"><b>" . $lang_forums ['text_next'] . " &gt;&gt;</b></font>\n";
	else
		$pager .= "<a href=\"" . htmlspecialchars ( "forums.php?" . $addparam . "&page=" . ($page + 1) ) . "\"><b>" . $lang_forums ['text_next'] . " &gt;&gt;</b></a>\n";
	
	$pagerstr = join ( " | ", $pagerarr );
	$pagertop = "<p align=\"center\" class=\"pagertop\">" . $pager . "<br />" . $pagerstr . "</p>\n";
	...
	print ("<h1 align=\"center\" class=\"pagetitle\"><a class=\"faqlink\" href=\"forums.php\">" . $SITENAME . "&nbsp;" . $lang_forums ['text_forums'] . "</a>--><a class=\"faqlink\" href=\"" . htmlspecialchars ( "forums.php?action=viewforum&forumid=" . $forumid ) . "\">" . $forumname . "</a><b>--></b><span id=\"top\">" . $subject . ($locked ? "&nbsp;&nbsp;<b>[<font class=\"striking\">" . $lang_forums ['text_locked'] . "</font>]</b>" : "") . "</span></h1>\n") ;
	end_main_frame ();
	print ($pagertop) ;
	...
	$row_marked=mysql_fetch_array(sql_query("select * from marked_topic where uid = $CURUSER[id] and tid = $topicid"));
	if($row_marked)
	print ("<a href=\"" . htmlspecialchars ( "marked_topic.php?action=delete&topicid=" . $topicid ) . "\"><input class=\"btn\" type=\"button\" value=\"取消收藏\" alt=\"deMark\"  /></a>");
	else
	print ("<a href=\"" . htmlspecialchars ( "marked_topic.php?action=add&topicid=" . $topicid ) . "\"><input class=\"btn\" type=\"button\" value=\"收藏本帖\" alt=\"Mark\"  /></a>");
	if ($maypost) {
		print ("<a href=\"" . htmlspecialchars ( "forums.php?action=reply&topicid=" . $topicid ) . "\"><img class=\"f_reply\" src=\"pic/trans.gif\" alt=\"Add Reply\" title=\"" . $lang_forums ['title_reply_directly'] . "\" /></a>&nbsp;&nbsp;") ;
	}
	print ("</td>") ;
	print ("</tr></table>\n") ;
	begin_frame ();
	...
	while ( $arr = mysql_fetch_assoc ( $res ) ) {
		...
		if ($authorid)
			print ("<a href=\"forums.php?action=viewtopic&topicid=" . $topicid . "\">" . $lang_forums ['text_view_all_posts'] . "</a>") ;
		else
			print ("<a href=\"" . htmlspecialchars ( "forums.php?action=viewtopic&topicid=" . $topicid . "&authorid=" . $posterid ) . "\">" . $lang_forums ['text_view_this_author_only'] . "</a>") ;
		...
		$body = "<div id=\"pid" . $postid . "body\" style=\"word-wrap: break-word; overflow: hidden; width= 80%;\">" . format_comment ( $arr ["body"] );
		...
		if (is_valid_id ( $arr ['editedby'] )) {
			...
			$body .= "<br /><br /><br /><br /><p><font class=\"small\">" . $lang_forums ['text_last_edited_by'] . get_username ( $arr ['editedby'] ) . $lang_forums ['text_last_edit_at'] . $lastedittime . "</font></p>\n";
		}
		$body .= "</div>";
		if ($signature)
		...
		if ($maypost) {
			print ('<a onclick="javascript:quick_reply_to(\'' . $arr2 ["username"] . '\',\'' . ($pn + $offset) . '\');" href="#quickreply"><img class="f_reply" src="pic/trans.gif" alt="Reply" title="回复该用户"></a>') ;
			print ("<a href=\"" . htmlspecialchars ( "forums.php?action=quotepost&postid=" . $postid ) . "\"><img class=\"f_quote\" src=\"pic/trans.gif\" alt=\"Quote\" title=\"" . $lang_forums ['title_reply_with_quote'] . "\" /></a>") ;
		}
		if (get_user_class () >= $postmanage_class || $is_forummod)
			print ("<a href=\"" . htmlspecialchars ( "forums.php?action=deletepost&postid=" . $postid ) . "\"><img class=\"f_delete\" src=\"pic/trans.gif\" alt=\"Delete\" title=\"" . $lang_forums ['title_delete_post'] . "\" /></a>") ;
		
		if (($CURUSER ["id"] == $posterid && ! $locked) || get_user_class () >= $postmanage_class || $is_forummod)
			print ("<a href=\"" . htmlspecialchars ( "forums.php?action=editpost&postid=" . $postid ) . "\"><img class=\"f_edit\" src=\"pic/trans.gif\" alt=\"Edit\" title=\"" . $lang_forums ['title_edit_post'] . "\" /></a>") ;
		print ("</td></tr></table>") ;
	}
...
<option style='background-color: white' value=\"40\">White</option>") ;
	...
	print ($pagerbottom) ;
	if ($maypost) {
		print ("<br /><a name='quickreply' id='quickreply'> </a><table style='border:1px solid #000000;'><tr>" . "<td class=\"text\" align=\"center\"><b>" . $lang_forums ['text_quick_reply'] . "</b><br /><br />" . "<form id=\"compose\" name=\"compose\" method=\"post\" action=\"?action=post\" onsubmit=\"return postvalid(this);\">" . "<input type=\"hidden\" name=\"id\" value=\"" . $topicid . "\" /><input type=\"hidden\" name=\"type\" value=\"reply\" /><br />") ;
		quickreply ( 'compose', 'body', $lang_forums ['submit_add_reply'] );
		print ("</form></td></tr></table>") ;
		print ("<p align=\"center\"><a class=\"index\" href=\"" . htmlspecialchars ( "forums.php?action=reply&topicid=" . $topicid ) . "\">" . $lang_forums ['text_add_reply'] . "</a></p>\n") ;
	} elseif ($locked)
...
function quick_reply_to(username,floor)
{
    ...
    parent.document.getElementById("quickreplytext").value = "回复 " + floor + " 楼 [@" + username + "] : "+parent.document.getElementById("quickreplytext").value;	
}
</script>') ;
...
if ($action == "deletetopic") {
	...
	$sure = 0 + $_GET ["sure"];
	if (! $sure) {
		stderr ( $lang_forums ['std_delete_topic'], $lang_forums ['std_delete_topic_note'] . "<a class=altlink href=forums.php?action=deletetopic&topicid=$topicid&sure=1>" . $lang_forums ['std_here_if_sure'], false );
	}
...
if ($action == "deletepost") {
	...
	if (mysql_num_rows ( $res ) == 0) // This is the first post of a topic
		stderr ( $lang_forums ['std_error'], $lang_forums ['std_cannot_delete_post'] . "<a class=altlink href=forums.php?action=deletetopic&topicid=$topicid&sure=1>" . $lang_forums ['std_delete_topic_instead'], false );
	else {
		$arr = mysql_fetch_row ( $res );
	...
	if (! $sure) {
		stderr ( $lang_forums ['std_delete_post'], $lang_forums ['std_delete_post_note'] . "<a class=altlink href=forums.php?action=deletepost&postid=$postid&sure=1>" . $lang_forums ['std_here_if_sure'], false );
	}
	...
	$Cache->delete_value ( 'user_' . $userid . '_post_count' );
	$Cache->delete_value ( 'topic_' . $topicid . '_post_count' );
	$Cache->delete_value ( 'post_' . $postid . '_post_id' );
	$Cache->delete_value ( 'post_' . $postid . '_post_name' );
...
if ($action == "viewforum") {
	...
	begin_main_frame ( "", true );
	print ("<h1 align=\"center\" class=\"pagetitle\"><a class=\"faqlink\" href=\"forums.php\">" . $SITENAME . "&nbsp;" . $lang_forums ['text_forums'] . "</a>--><a class=\"faqlink\" href=\"" . htmlspecialchars ( "forums.php?action=viewforum&forumid=" . $forumid ) . "\">" . $forumname . "</a></h1>\n") ;
	end_main_frame ();
	// print("<br />");
	...
	if ($maypost)
		print ("<a href=\"" . htmlspecialchars ( "forums.php?action=newtopic&forumid=" . $forumid ) . "\"><img class=\"f_new\" src=\"pic/trans.gif\" alt=\"New Topic\" title=\"" . $lang_forums ['title_new_topic'] . "\" /></a>&nbsp;&nbsp;") ;
	print ("</td>") ;
	print ("</tr></table>\n") ;
	if ($numtopics > 0) {
		
		if (is_forum_moderator ( $forumid, 'forum' ) || get_user_class () >= $postmanage_class) {
			print ('<form id="topics_form" action="forums.php?action=multijob&forumid=' . $forumid . '" method="post">') ;
		}
		
		print ("<table border=\"1\" cellspacing=\"0\" cellpadding=\"5\" width=\"100%\">") ;
		
		print ("<tr><td class=\"colhead\" align=\"center\" width=\"99%\">" . $lang_forums ['col_topic'] . "</td><td class=\"colhead\" align=\"center\"><a href=\"" . htmlspecialchars ( "forums.php?action=viewforum&forumid=" . $forumid . $addparam . "&sort=" . ($_GET ["sort"] == 'firstpostdesc' ? "firstpostasc" : "firstpostdesc") ) . "\" title=\"" . ($_GET ["sort"] == 'firstpostdesc' ? $lang_forums ['title_order_topic_asc'] : $lang_forums ['title_order_topic_desc']) . "\">" . $lang_forums ['col_author'] . "</a></td><td class=\"colhead\" align=\"center\">" . $lang_forums ['col_replies'] . "/" . $lang_forums ['col_views'] . "</td><td class=\"colhead\" align=\"center\"><a href=\"" . htmlspecialchars ( "forums.php?action=viewforum&forumid=" . $forumid . $addparam . "&sort=" . ($_GET ["sort"] == 'lastpostasc' ? "lastpostdesc" : "lastpostasc") ) . "\" title=\"" . ($_GET ["sort"] == 'lastpostasc' ? $lang_forums ['title_order_post_desc'] : $lang_forums ['title_order_post_asc']) . "\">" . $lang_forums ['col_last_post'] . "</a></td>\n") ;
		...
		while ( $topicarr = mysql_fetch_assoc ( $topicsres ) ) {
			...
			if ($tpages > 1) {
				...
				for($i = 1; $i <= $tpages; ++ $i) {
					...
					$topicpages .= " <a href=\"" . htmlspecialchars ( "forums.php?action=viewtopic&topicid=" . $topicid . "&page=" . ($i - 1) ) . "\">$i</a>";
				}
			...
			$subject = ($sticky ? "<img class=\"sticky\" src=\"pic/trans.gif\" alt=\"Sticky\" title=\"" . $lang_forums ['title_sticky'] . "\" />&nbsp;&nbsp;" : "") . "<a href=\"" . htmlspecialchars ( "forums.php?action=viewtopic&forumid=" . $forumid . "&topicid=" . $topicid ) . "\" " . $onmouseover . ">" . highlight_topic ( highlight ( $search, htmlspecialchars ( $topicarr ["subject"] ) ), $hlcolor ) . "</a>" . $topicpages;
			...
			if ($lastpostread >= $lppostid)
				...
				if ($lastpostread != $CURUSER ['last_catchup'])
					$subject .= "&nbsp;&nbsp;<a href=\"" . htmlspecialchars ( "forums.php?action=viewtopic&forumid=" . $forumid . "&topicid=" . $topicid . "&page=p" . $lastpostread . "#pid" . $lastpostread ) . "\" title=\"" . $lang_forums ['title_jump_to_unread'] . "\"><font class=\"small new\"><b>" . $lang_forums ['text_new'] . "</b></font></a>";
			}
		  ...
		  // width=\"940\">");
		if (is_forum_moderator ( $forumid, 'forum' ) || get_user_class () >= $postmanage_class) {
			print ('<tr><td align=\"left\" colspan="100"><a href="#" onclick="set_checked_topics(true); return false;">全选</a> <a href="#" onclick="set_checked_topics(false); return false;">全不选</a>, 选中项: <input type="submit" name="job" value="删除"/> <input type="submit" name="job" value="锁定"/> <input type="submit" name="job" value="解锁"/></form></td></tr>') ;
		}
		...
		print ("</td>") ;
		?>
<td align="left" colspan="3"><span id="order" onclick="dropmenu(this);"><span
		style="cursor: pointer;"><b><?php echo $lang_forums['text_order'] ?></b></span>
		<span id="orderlist" class="dropmenu" style="display: none"><ul>
				<li><a
					href="forums.php?action=viewforum&amp;forumid=<?php echo $forumid . $addparam ?>&amp;sort=firstpostdesc"><?php echo $lang_forums['text_topic_desc'] ?></a></li>
				<li><a
					href="forums.php?action=viewforum&amp;forumid=<?php echo $forumid . $addparam ?>&amp;sort=firstpostasc"><?php echo $lang_forums['text_topic_asc'] ?></a></li>
				<li><a
					href="forums.php?action=viewforum&amp;forumid=<?php echo $forumid . $addparam ?>&amp;sort=lastpostdesc"><?php echo $lang_forums['text_post_desc'] ?></a></li>
				<li><a
					href="forums.php?action=viewforum&amp;forumid=<?php echo $forumid . $addparam ?>&amp;sort=lastpostasc"><?php echo $lang_forums['text_post_asc'] ?></a></li>
			</ul> </span> </span></td>
<?php
		print ("</tr>") ;
		print ("</table>") ;
		if (is_forum_moderator ( $forumid, 'forum' ) || get_user_class () >= $postmanage_class) {
			print ('<script type="text/javascript">function set_checked_topics(val){var checkboxs=document.getElementsByName("tids[]"); for (var i=0; i<checkboxs.length; i++) checkboxs[i].checked=val; }</script>') ;
		}
...
if ($action == "viewunread") {
	...
	print ("<h1 align=\"center\" class=\"pagetitle\"><a class=\"faqlink\" href=\"forums.php\">" . $SITENAME . "&nbsp;" . $lang_forums ['text_forums'] . "</a>-->" . $lang_forums ['text_topics_with_unread_posts'] . "</h1>") ;
	...
	while ( $arr = mysql_fetch_assoc ( $res ) ) {
		...
		print ("<tr><td class=\"rowfollow\" align=\"left\"><table border=\"0\" cellspacing=\"0\" cellpadding=\"0\"><tr><td class=\"embedded\" style='padding-right: 10px'>" . get_topic_image ( "unread" ) . "</td><td class=\"embedded\">" . "<a href=\"" . htmlspecialchars ( "forums.php?action=viewtopic&topicid=" . $topicid . ($lastpostread > 0 && $lastpostread != $CURUSER ['last_catchup'] ? "&page=p" . $lastpostread . "#pid" . $lastpostread : "") ) . "\">" . highlight_topic ( htmlspecialchars ( $arr ["subject"] ), $arr ["hlcolor"] ) . "</a></td></tr></table></td><td class=\"rowfollow\" align=\"left\"><a href=\"" . htmlspecialchars ( "forums.php?action=viewforum&forumid=" . $forumid ) . "\"><b>" . $forumname . "</b></a></td></tr>\n") ;
	}
	if ($n > 0) {
		print ("</table>\n") ;
				...
				<tbody>
					...
					<tr>
						<td style="padding-bottom: 3px; border: 0;" valign="top"><input
							name="keywords" type="text" value="<?php echo $keywords ?>"
...
begin_main_frame ();
print ("<h1 align=\"center\" class=\"pagetitle\">" . $SITENAME . "&nbsp;" . $lang_forums ['text_forums'] . "</h1>") ;
print ("<p align=\"center\"><a href=\"forums.php?action=search\"><b>" . $lang_forums ['text_search'] . "</b></a> | <a href=\"forums.php?action=viewunread\"><b>" . $lang_forums ['text_view_unread'] . "</b></a> | <a href=\"forums.php?catchup=1\"><b>" . $lang_forums ['text_catch_up'] . "</b></a>| <a href=\"marked_topic.php\"><b>收藏</b></a> " . (get_user_class () >= $forummanage_class ? "| <a href=\"forummanage.php\"><b>" . $lang_forums ['text_forum_manager'] . "</b></a>" : "") . "</p>") ;
print ("<table border=\"1\" cellspacing=\"0\" cellpadding=\"5\" width=\"100%\">\n") ;
...
foreach ( $overforums as $a ) {
	...
	foreach ( $forums as $forums_arr ) {
		...
		if ($arr) {
			...
			$lastpost = "<a href=\"" . htmlspecialchars ( "forums.php?action=viewtopic&topicid=" . $lasttopicid . "&page=last#last" ) . "\" title=\"" . htmlspecialchars ( $lasttopicsubject ) . "\">" . $lasttopic . "</a><br />" . $lastpostdate . "&nbsp;|&nbsp;" . get_username ( $lastposterid );
		...
		if ($posttodaycount > 0)
			...
			$posttoday = "";
		print ("<tr><td class=\"rowfollow\" align=\"left\"><table border=\"0\" cellspacing=\"0\" cellpadding=\"0\"><tr><td class=\"embedded\" style='padding-right: 10px'>" . $img . "</td><td class=\"embedded\"><a href=\"" . htmlspecialchars ( "forums.php?action=viewforum&forumid=" . $forumid ) . "\"><font class=\"big\"><b>" . $forumname . "</b></font></a>" . $posttoday . "<br />" . $forumdescription . "</td></tr></table></td><td class=\"rowfollow\" align=\"center\" width=\"1%\">" . $topiccount . "</td><td class=\"rowfollow\" align=\"center\" width=\"1%\">" . $postcount . "</td>" . "<td class=\"rowfollow nowrap\" align=\"left\">" . $lastpost . "</td><td class=\"rowfollow\" align=\"left\">" . $forummoderators . "</td></tr>\n") ;
	}
	$count ++;
}
// End Table Mod
